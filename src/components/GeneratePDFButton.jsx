...
function GeneratePDFButton({ invoice }) {
  ...
  const generatePDF = () => {
    ...
    // if (invoice.pc1 > 0) {
    //   doc.text(`PC1: ${invoice.pc1} Cfa`, 35, currentYv)
    //   currentYv += 5 // Ajustez selon l'espacement désiré entre les lignes
    // }

    // if (invoice.pc2 > 0) {
    //   doc.text(`PC2: ${invoice.pc2} Cfa`, 35, currentYv)
    //   currentYv += 5 // Ajustez selon l'espacement désiré entre les lignes
    // }

    // if (invoice.deplacement > 0) {
    //   doc.text(`Deplacement: ${invoice.deplacement} Cfa`, 35, currentYv)
    //   currentYv += 5 // Ajustez selon l'espacement désiré entre les lignes
    // }

    // doc.text(`Patient: ${invoice.prixPatient || '0'} Cfa `, 35, currentY + 12)
    // doc.text(
    //   `Tiers Payant: ${invoice.prixPartenaire  || '0'} Cfa `,
    //   35,
    //   currentY + 17
    // )
    // doc.text(`Couverture : ${invoice.pourcentageCouverture || '0'}% `, 35, currentY + 22)
    // doc.text(`${invoice.partenaireId?.typePartenaire || 'Paf'} : ${invoice.partenaireId?.nom || 'Sans assurance ni ipm'} `, 35, currentY + 27)
    // let suffixeReduction = '' // Initialiser le suffixe vide

    // // Déterminer le suffixe en fonction de typeReduction
    // if (invoice.typeReduction === 'montant') {
    //   suffixeReduction = ' Cfa'
    // } else if (invoice.typeReduction === 'pourcentage') {
    //   suffixeReduction = ' %'
    // }else {
    //   suffixeReduction = ''
    // }

    // // Construire et afficher le texte avec le suffixe approprié
    // doc.text(`Reduction : ${invoice.reduction || '0'}${suffixeReduction}`, 35, currentY + 32)
    ...
    //   if (invoice.partenaireId?.typePartenaire === 'assurance' && test.prixAssurance !== undefined) {
    //     prixChoisi = test.prixAssurance
    //   } else if (invoice.partenaireId?.typePartenaire === 'ipm' && test.prixIpm !== undefined) {
    //     prixChoisi = test.prixIpm
    //   } else if (test.PrixPaf !== undefined) { // Vérifiez que c'est la bonne propriété
    ...
    //   doc.text(`${coeficiantB}`, 110, textY) // Coefficient B
    //   // doc.text(`${prixChoisi.toFixed(0)}`, 138, textY) // Prix choisi

    //   // Calculer et afficher total
    //   const total = prixChoisi * test.coeficiantB
    ...
    //     currentY += 7 // Espacer pour le prochain test
    //   }
    // })
